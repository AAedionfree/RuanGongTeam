...
function load_dai(request_url) {
	...
	function yes_arrange(logid, request_url) {
		...
		mui.plusReady(function() {
			...
			// }
			// alert(s);
			var arrange_yes_url = request_url + 'devDealScrapRecord?userAccount=' + user.userAccount + '&logId=' + logid +
				'&logStatus=1';
			// alert(search_url);
			mui.ajax({
				type: 'GET',
	...
	function no_arrange(logid, request_url) {
		...
		mui.plusReady(function() {
			...
			// }
			// alert(s);
			var arrange_no_url = request_url + 'devDealScrapRecord?userAccount=' + user.userAccount + '&logId=' + logid +
				'&logStatus=2';
			mui.ajax({
				type: 'GET',
				url: arrange_no_url,
